...
const liveSuggestionsEl = document.getElementById('liveSuggestions');
...
async function sendMessage() {
    ...
    try {
        ...
        if (res.status === 429) {
            addBotMessage('You\'re sending messages too fast! Please wait a minute before trying again. (Limit: 30 messages/min)');
        } else if (data.response) {
            addBotMessage(data.response);

            // Feature 5 — Did You Mean? spelling correction suggestion
            if (data.did_you_mean) {
                const dym = data.did_you_mean;
                setTimeout(() => {
                    addBotMessage(
                        `Did you mean: <span class="dym-link" onclick="setInputAndSend(${JSON.stringify(dym)})" ` +
                        `style="color:var(--primary);text-decoration:underline;cursor:pointer;font-weight:600;">${escapeHtml(dym)}</span>?`
                    );
                }, 300);
            }
...
function exportChat() {
    ...
    let content = `JSPM Wagholi Campus — ${botName} Chat Export\n`;
    ...
    messages.forEach(msg => {
        ...
        const sender = isUser ? 'You' : botName;
    ...
    content += '─'.repeat(60) + '\n';
    content += 'Generated by JSPM Wagholi CampusBot & MindMate AI\n';
...
// ─── Live FAQ Suggestions While Typing (Feature 7) ────────
// Only active on campus bot (BOT_TYPE === 'campus')

(function initLiveSuggestions() {
    if (typeof BOT_TYPE === 'undefined' || BOT_TYPE !== 'campus') return;
    if (!liveSuggestionsEl || !chatInput) return;
    ...
    chatInput.addEventListener('input', function () {
        ...
        debounceTimer = setTimeout(async function () {
            try {
                ...
                const lang = langEl ? (langEl.value === 'auto' ? 'en' : langEl.value) : 'en';
                const res = await fetch(`/api/suggestions?q=${encodeURIComponent(query)}&language=${lang}`);
                const data = await res.json();

                if (data.suggestions && data.suggestions.length > 0) {
                    showLiveSuggestions(data.suggestions);
                }
            } catch (e) {
                // Silently fail — live suggestions are non-critical
            }
        }, 300);
    });

    // Hide dropdown when input is cleared or on blur (with delay for click)
    chatInput.addEventListener('blur', function () {
        setTimeout(hideLiveSuggestions, 180);
    });
    chatInput.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') hideLiveSuggestions();
    });
})();

function showLiveSuggestions(suggestions) {
    if (!liveSuggestionsEl) return;
    liveSuggestionsEl.innerHTML = suggestions.map(q =>
        `<div class="live-suggestion-item" onmousedown="pickLiveSuggestion(${JSON.stringify(q)})">${escapeHtml(q)}</div>`
    ).join('');
    liveSuggestionsEl.style.display = 'block';
}

function hideLiveSuggestions() {
    if (!liveSuggestionsEl) return;
    liveSuggestionsEl.style.display = 'none';
    liveSuggestionsEl.innerHTML = '';
}

function pickLiveSuggestion(text) {
    chatInput.value = text;
    hideLiveSuggestions();
    chatInput.focus();
}
